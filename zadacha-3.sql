alter table employee add status varchar2(25) default 'NORMAL' not null;

alter table employee add status_changed date;
...
select * from employee where status = 'FIRED';

-- Да се реализира листинг на всички служители които са в майчинство
-- в момента.


select * from employee where status = 'MATERNITY';

-- Да се реализира листинг на всички служители които са в отпуска
-- / болничен в момента

select * from employee where status = 'LEISURE_LEAVE' or status = 'SICK_LEAVE';
...
AND salary > 5000 order by first_name asc;
